...
#     via the Leaf-2 side of the P2P link (192.168.255.1).
#  2. netplan apply
...
# Fabric edge link to Leaf-2 Eth1/4 (VRF tenant-1)
...
echo
info "pinging Leaf-2 edge ($LEAF_GW)..."
if ping -c 2 -W 2 "$LEAF_GW" >/dev/null 2>&1; then
  ok "Leaf-2 edge reachable"
else
  err "Leaf-2 edge $LEAF_GW unreachable"
  err "   Check: (a) EVE-NG cable from frontend:e1 to Leaf-2:Eth1/4 is drawn"
  err "          (b) Leaf-2 Eth1/4 was configured (run configure_leaf1_for_frontend.sh)"
...
if [[ $passed -eq $total ]]; then
  ...
  echo "   • Leaf-2 BGP to master should be Established:"
  echo "       sshpass -p admin ssh admin@10.1.1.1 'show bgp ipv4 unicast summary vrf tenant-1'"
  echo "   • VIPs should show *>e via 10.1.1.10 on Leaf-2:"
